...
train_idx <- createDataPartition(sleep_data$Has.Sleep.Disorder, p = 0.75, list = FALSE)
train_val_data <- sleep_data[train_idx, ]
test_data <- sleep_data[-train_idx, ]
...
preprocess_model_train <- preProcess(train_data, method = c("knnImpute"))
preprocess_model_val <- preProcess(val_data, method = c("knnImpute"))
preprocess_model_test <- preProcess(test_data, method = c("knnImpute"))

# apply preprocess to train, val, and test sets
train_data <- predict(preprocess_model_train, newdata = train_data)
val_data <- predict(preprocess_model_val, newdata = val_data)
test_data <- predict(preprocess_model_test, newdata = test_data)
```
