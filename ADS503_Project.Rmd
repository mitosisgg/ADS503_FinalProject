...
# Stratified split: 75% for training, 25% for testing
split_index <- createDataPartition(sleep_data$Has.Sleep.Disorder, p = 0.75, list = FALSE)
train_data <- sleep_data[split_index, ]
test_data <- sleep_data[-split_index, ]
...
# Determine maximum number of weights needed
...
# Evaluate performance
rf_cm <- confusionMatrix(rf_preds, y_test, positive = "Yes")
...
rf_roc <- roc(response = y_test, predictor = rf_probs)
rf_auc <- auc(rf_roc)
...
# Plot ROC Curve
plot(rf_roc, main = "Random Forest ROC Curve (Test Set)", col = "darkgreen", lwd = 3, xlim = c(1,0), ylim = c(0,1))
abline(a = 0, b = 1, lty = 2, col = "gray")
